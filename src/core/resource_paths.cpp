...
std::filesystem::path sourceAssetsRoot() {
  return std::filesystem::path(NOCTALIA_GREETER_ASSETS_DIR);
}
...
void appendUnique(std::vector<std::filesystem::path> &candidates,
...
std::vector<std::filesystem::path> assetCandidates() {
  ...
  if (const char *env = std::getenv("NOCTALIA_GREETER_ASSETS_DIR");
      env != nullptr && env[0] != '\0') {
    ...
    kLog.warn("NOCTALIA_GREETER_ASSETS_DIR is not a valid asset bundle: {}",
              overridePath.string());
  }
  ...
  if (auto exe = executablePath()) {
    ...
    appendUnique(candidates, exeDir.parent_path() / "share" /
                                 "noctalia-greeter" / "assets");
  }
...
std::filesystem::path resolveAssetsRoot() {
  ...
  kLog.warn("could not locate a valid asset bundle; defaulting to {}",
            fallback.string());
  return fallback;
}
