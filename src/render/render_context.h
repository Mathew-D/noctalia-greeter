...
public:
  ...
  [[nodiscard]] const RenderBackend &backend() const noexcept {
    return *m_backend;
  }

  // Renderer interface, used by widgets for measurement and textures
  [[nodiscard]] TextMetrics
  measureText(std::string_view text, float fontSize, bool bold = false,
  ...
  [[nodiscard]] TextMetrics measureFont(float fontSize,
                                        bool bold = false) override;
  void measureTextCursorStops(std::string_view text, float fontSize,
                              const std::vector<std::size_t> &byteOffsets,
                              std::vector<float> &outStops,
                              bool bold = false) override;
  [[nodiscard]] TextMetrics measureGlyph(char32_t codepoint,
  ...
  [[nodiscard]] std::uint64_t textMetricsGeneration() const noexcept override {
    return m_textMetricsGeneration;
  }

private:
  void makeCurrentNoSurface();
  void renderNode(const Node *node, const Mat3 &parentTransform,
                  float parentOpacity, float sw, float sh, float bw, float bh,
                  float clipLeft, float clipTop, float clipRight,
                  float clipBottom, bool hasClip);
