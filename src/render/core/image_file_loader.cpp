...
void argb32ToRgba(const unsigned char *src, int srcStride, std::uint8_t *dst,
                  int width, int height) {
  for (int y = 0; y < height; ++y) {
    const std::uint32_t *row =
        reinterpret_cast<const std::uint32_t *>(src + (y * srcStride));
    std::uint8_t *outRow = dst + (static_cast<std::size_t>(y) *
...
rasterizeSvg(const std::vector<std::uint8_t> &fileData, int targetSize,
             std::string *errorMessage) {
  GError *gerror = nullptr;
  RsvgHandle *handle =
      rsvg_handle_new_from_data(fileData.data(), fileData.size(), &gerror);
  if (handle == nullptr) {
    if (errorMessage != nullptr) {
      *errorMessage = std::string("failed to parse SVG: ") +
                      (gerror != nullptr ? gerror->message : "unknown");
    }
    if (gerror != nullptr) {
      g_error_free(gerror);
    }
  ...
  gboolean hasIntrinsic = rsvg_handle_get_intrinsic_size_in_pixels(
      handle, &intrinsicW, &intrinsicH);
  if (hasIntrinsic == FALSE || intrinsicW <= 0.0 || intrinsicH <= 0.0) {
    ...
    rsvg_handle_get_intrinsic_dimensions(handle, &outHasW, &outW, &outHasH,
                                         &outH, &outHasViewbox, &outViewbox);
    if (outHasViewbox == TRUE && outViewbox.width > 0.0 &&
        outViewbox.height > 0.0) {
  ...
  if (rsvg_handle_render_document(handle, cr, &viewport, &renderError) ==
      FALSE) {
    if (errorMessage != nullptr) {
      *errorMessage =
          std::string("failed to render SVG: ") +
          (renderError != nullptr ? renderError->message : "unknown");
    }
  ...
  LoadedImageFile loaded{
      .rgba = std::vector<std::uint8_t>(static_cast<std::size_t>(width) *
                                        static_cast<std::size_t>(height) * 4U),
      .width = width,
      .height = height,
  };
  argb32ToRgba(cairo_image_surface_get_data(surface),
               cairo_image_surface_get_stride(surface), loaded.rgba.data(),
               width, height);
...
std::optional<LoadedImageFile> loadImageFile(const std::string &path,
                                             int targetSize,
                                             std::string *errorMessage) {
  if (path.empty()) {
    if (errorMessage != nullptr) {
  ...
  if (auto decoded =
          decodeRasterImage(fileData.data(), fileData.size(), errorMessage)) {
    ...
    if (targetSize > 0 && maxDim > targetSize && width > 0 && height > 0) {
      const float scale =
          static_cast<float>(targetSize) / static_cast<float>(maxDim);
      const int resizedW = std::max(
          1, static_cast<int>(std::lround(static_cast<float>(width) * scale)));
      const int resizedH = std::max(
          1, static_cast<int>(std::lround(static_cast<float>(height) * scale)));

      std::vector<std::uint8_t> resized(static_cast<std::size_t>(resizedW) *
                                        static_cast<std::size_t>(resizedH) *
                                        4U);
      unsigned char *result = stbir_resize_uint8_linear(
          pixels.data(), width, height, 0, resized.data(), resizedW, resizedH,
          0, STBIR_RGBA);
      if (result != nullptr) {
    ...
    return LoadedImageFile{
        .rgba = std::move(pixels), .width = width, .height = height};
  }
