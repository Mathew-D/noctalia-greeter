...
bool containsColorGlyph(std::string_view text) {
  ...
  while (i < n) {
    ...
    if (b < 0x80) {
      ...
      cp = static_cast<char32_t>((b & 0x0F) << 12 | (s[i + 1] & 0x3F) << 6 |
                                 (s[i + 2] & 0x3F));
      len = 3;
    } else if ((b & 0xF8) == 0xF0 && i + 3 < n) {
      cp = static_cast<char32_t>((b & 0x07) << 18 | (s[i + 1] & 0x3F) << 12 |
...
bool CairoTextRenderer::CacheKey::operator==(
    const CacheKey &other) const noexcept {
  return bold == other.bold && sizeQ == other.sizeQ && scaleQ == other.scaleQ &&
         maxWidthQ == other.maxWidthQ && maxLines == other.maxLines &&
         align == other.align && colorRgba == other.colorRgba &&
         text == other.text && fontFamily == other.fontFamily;
}

std::size_t
CairoTextRenderer::CacheKeyHash::operator()(const CacheKey &k) const noexcept {
  ...
  hashCombine(seed,
              std::hash<std::uint8_t>{}(static_cast<std::uint8_t>(k.align)));
  hashCombine(seed, std::hash<std::uint32_t>{}(k.colorRgba));
  hashCombine(seed, std::hash<bool>{}(k.bold));
  return seed;
}

bool CairoTextRenderer::MetricsKey::operator==(
    const MetricsKey &other) const noexcept {
  return bold == other.bold && sizeQ == other.sizeQ && scaleQ == other.scaleQ &&
         maxWidthQ == other.maxWidthQ && maxLines == other.maxLines &&
         align == other.align && text == other.text &&
         fontFamily == other.fontFamily;
}

std::size_t CairoTextRenderer::MetricsKeyHash::operator()(
    const MetricsKey &k) const noexcept {
  std::size_t seed = std::hash<std::string>{}(k.text);
  hashCombine(seed, std::hash<std::string>{}(k.fontFamily));
  hashCombine(seed, std::hash<std::uint32_t>{}(k.sizeQ));
  hashCombine(seed, std::hash<std::uint32_t>{}(k.maxWidthQ));
...
void CairoTextRenderer::initialize(RenderBackend *backend,
  ...
  if (cairo_version() < CAIRO_VERSION_ENCODE(1, 18, 0)) {
    kLog.warn("cairo version {} (<1.18), COLR v1 color emoji will not render",
...
PangoLayout *CairoTextRenderer::buildLayout(std::string_view text,
                                            float fontSize, bool bold,
                                            float maxWidthPxScaled,
  ...
  pango_font_description_set_family(desc, fontFamilyStr.empty()
                                              ? m_fontFamily.c_str()
                                              : fontFamilyStr.c_str());
  pango_font_description_set_weight(desc, bold ? PANGO_WEIGHT_BOLD
                                               : PANGO_WEIGHT_NORMAL);
  pango_font_description_set_absolute_size(
      desc, static_cast<double>(rasterSize) * PANGO_SCALE);
  ...
  if (maxWidthPxScaled > 0.0f) {
    ...
    pango_layout_set_width(layout,
                           static_cast<int>(maxWidthPxScaled * PANGO_SCALE));
...
CairoTextRenderer::metricsFromLayout(PangoLayout *layout) const {
  PangoRectangle ink;
  ...
  // top.
  const float ascent =
      static_cast<float>(baselinePango - logical.y) * pscale * invScale;
  const float descent =
      static_cast<float>(logical.height - (baselinePango - logical.y)) *
      pscale * invScale;
  const float inkTop =
      static_cast<float>(ink.y - baselinePango) * pscale * invScale;
  const float inkBottom =
      static_cast<float>(ink.y + ink.height - baselinePango) * pscale *
      invScale;
  const float inkLeft = static_cast<float>(ink.x) * pscale * invScale;
  const float inkRight =
      static_cast<float>(ink.x + ink.width) * pscale * invScale;
...
CairoTextRenderer::TextMetrics
CairoTextRenderer::measure(std::string_view text, float fontSize, bool bold,
  ...
  PangoLayout *layout =
      buildLayout(text, fontSize, bold, maxWidth * m_contentScale, maxLines,
                  align, fontFamily);
...
CairoTextRenderer::TextMetrics CairoTextRenderer::measureFont(float fontSize,
                                                              bool bold) const {
  if (m_pangoContext == nullptr) {
    return {};
  }
  ...
  PangoFontMetrics *metrics = pango_context_get_metrics(
      m_pangoContext, desc, pango_language_get_default());
  pango_font_description_free(desc);
  if (metrics == nullptr) {
    return {};
  }
  ...
  const float pscale = 1.0f / static_cast<float>(PANGO_SCALE);
  const float ascent =
      static_cast<float>(pango_font_metrics_get_ascent(metrics)) * pscale *
      invScale;
  const float descent =
      static_cast<float>(pango_font_metrics_get_descent(metrics)) * pscale *
      invScale;
...
void CairoTextRenderer::measureCursorStops(
  ...
  PangoLayout *layout =
      buildLayout(text, fontSize, bold, 0.0f, 0, TextAlign::Start);
  const float invScale = 1.0f / m_contentScale;
  const float pscale = 1.0f / static_cast<float>(PANGO_SCALE);
  for (const std::size_t offset : byteOffsets) {
    const std::size_t clampedOffset = std::min(offset, text.size());
    const int index = clampedOffset > static_cast<std::size_t>(
...
void CairoTextRenderer::rasterizeLayout(PangoLayout *layout, const Color &color,
                                        bool tinted, CacheEntry &entry) {
  entry.tinted = tinted;
  ...
  const int extraLeftPx =
      (std::max(0, logicalLayout.x - inkLayout.x) + PANGO_SCALE - 1) /
      PANGO_SCALE;
  const int extraRightPx =
      (std::max(0, (inkLayout.x + inkLayout.width) -
                       (logicalLayout.x + logicalLayout.width)) +
       PANGO_SCALE - 1) /
      PANGO_SCALE;
  const int extraTopPx =
      (std::max(0, logicalLayout.y - inkLayout.y) + PANGO_SCALE - 1) /
      PANGO_SCALE;
  const int extraBottomPx =
      (std::max(0, (inkLayout.y + inkLayout.height) -
                       (logicalLayout.y + logicalLayout.height)) +
       PANGO_SCALE - 1) /
      PANGO_SCALE;
  ...
  const int baselinePango = pango_layout_get_baseline(layout);
  entry.baselinePx =
      static_cast<float>(baselinePango) / static_cast<float>(PANGO_SCALE) +
      static_cast<float>(extraTopPx);
  ...
  if (pxWidth > maxTex) {
    kLog.warn("text width {}px exceeds backend texture limit {}, clipping",
              pxWidth, maxTex);
    pxWidth = maxTex;
  }
  ...
  std::vector<TilePlan> plan;
  {
    ...
    do {
      ...
      const int lineBottomPx =
          (logical.y + logical.height + PANGO_SCALE - 1) / PANGO_SCALE;
      const int lineBaselinePx =
          pango_layout_iter_get_baseline(iter) / PANGO_SCALE;
  ...
  for (const auto &tilePlan : plan) {
    ...
    for (const auto &ls : tilePlan.lines) {
      const double baselineInTile =
          static_cast<double>(ls.baselinePx - tilePlan.yTopPx);
      cairo_save(cr);
      cairo_translate(cr, static_cast<double>(ls.xLeftPx - blockLeftPx),
                      baselineInTile);
    ...
    tight.resize(static_cast<std::size_t>(tightRowBytes) *
                 static_cast<std::size_t>(tileH));
    for (int y = 0; y < tileH; ++y) {
      std::memcpy(tight.data() + y * tightRowBytes, data + y * stride,
                  static_cast<std::size_t>(tightRowBytes));
    }
    ...
    TextureHandle texture = m_textureManager->loadFromPixels(
        tight.data(), pxWidth, tileH,
        tinted ? TextureDataFormat::Alpha : TextureDataFormat::Rgba,
    ...
    entry.bytes += static_cast<std::size_t>(tightRowBytes) *
                   static_cast<std::size_t>(tileH);
  }
...
CairoTextRenderer::CacheEntry *
CairoTextRenderer::lookupOrRasterize(std::string_view text, float fontSize,
...
void CairoTextRenderer::draw(float surfaceWidth, float surfaceHeight, float x,
                             float baselineY, std::string_view text,
                             float fontSize, const Color &color,
                             const Mat3 &transform, bool bold, float maxWidth,
                             int maxLines, TextAlign align,
                             std::string_view fontFamily) {
  ...
  CacheEntry *entry = lookupOrRasterize(text, fontSize, bold, maxWidth,
                                        maxLines, align, color, fontFamily);
  if (entry == nullptr || entry->tiles.empty()) {
    return;
  }
  if (entry->tiles.size() > 1) {
    kLog.warn("draw tiles={} pxW={} pxH={} baseXY=({}, {})",
              entry->tiles.size(), entry->pixelWidth, entry->pixelHeight, x,
              baselineY);
  }
  ...
  const Mat3 localTranslation =
      Mat3::translation(x - inkOffX, baselineY - baselineLocal);
