...
constexpr EGLint kConfigAttrs[] = {EGL_SURFACE_TYPE, EGL_WINDOW_BIT, EGL_RENDERABLE_TYPE, EGL_OPENGL_ES2_BIT,
...
bool DebugWaylandPlatform::initialize() {
  ...
  m_eglSurface = eglCreateWindowSurface(m_eglDisplay, m_config, reinterpret_cast<EGLNativeWindowType>(m_window), nullptr);
...
void DebugWaylandPlatform::pointerEnter(void* data, wl_pointer*, uint32_t, wl_surface*, wl_fixed_t sx, wl_fixed_t sy) {
  auto* self = static_cast<DebugWaylandPlatform*>(data);
...
void DebugWaylandPlatform::pointerButton(void* data, wl_pointer*, uint32_t, uint32_t, uint32_t button, uint32_t state) {
  auto* self = static_cast<DebugWaylandPlatform*>(data);
...
void DebugWaylandPlatform::keyboardKey(void* data, wl_keyboard*, uint32_t, uint32_t, uint32_t key, uint32_t state) {
  auto* self = static_cast<DebugWaylandPlatform*>(data);
  if (self->m_view == nullptr || self->m_xkbState == nullptr || state != WL_KEYBOARD_KEY_STATE_PRESSED) {
    return;
  }
  ...
  if (sym == XKB_KEY_BackSpace) {
    self->m_view->keyBackspace();
    return;
  }
  if (sym == XKB_KEY_Return || sym == XKB_KEY_KP_Enter) {
    self->m_view->keyReturn();
    return;
  }
  if (sym == XKB_KEY_Tab) {
    self->m_view->keyTab();
  ...
  char buffer[64]{};
  const int bytes = xkb_state_key_get_utf8(self->m_xkbState, code, buffer, sizeof(buffer));
  if (bytes > 0) {
    self->m_view->textInput(std::string_view(buffer, static_cast<std::size_t>(bytes)));
  }
}
