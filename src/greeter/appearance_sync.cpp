...
[[nodiscard]] bool validatePaletteObject(const nlohmann::json &palette,
  ...
  for (const auto key : requiredPaletteKeys()) {
    ...
    if (it == palette.end() || !it->is_string()) {
      errorOut =
          std::string("palette is missing key '") + std::string(key) + "'";
      return false;
    }
  }
  return true;
...
[[nodiscard]] bool installRegularFile(const std::filesystem::path &source,
  ...
  std::filesystem::copy_file(source, destination,
                             ...
                             ec);
  if (ec) {
    errorOut = std::string("failed to copy '") + source.string() + "' to '" +
...
removeInstalledWallpapers(const std::filesystem::path &syncedDir,
  ...
  for (const auto &entry : std::filesystem::directory_iterator(syncedDir, ec)) {
    ...
    if (ec) {
      errorOut = std::string("failed to remove '") + entry.path().string() +
                 "': " + ec.message();
      return false;
    }
  }
...
stagingWallpaperFiles(const std::filesystem::path &stagingDirectory) {
  ...
  for (const auto &entry :
       std::filesystem::directory_iterator(stagingDirectory, ec)) {
...
bool validateStagingManifest(const std::filesystem::path &stagingDirectory,
                             std::string &errorOut) {
...
bool installFromStaging(const std::filesystem::path &stagingDirectory,
                        std::string &errorOut) {
  std::error_code ec;
  if (!std::filesystem::is_directory(stagingDirectory, ec) || ec) {
