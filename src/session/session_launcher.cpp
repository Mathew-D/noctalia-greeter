...
int SessionLauncher::launch(const system::User& user, const Session& session,
  ...
  if (debugMode) {
    core::log(core::LogLevel::Info, "session", std::format("--debug: authenticated launch skipped for {}", user.name));
    return 0;
  }
  ...
  core::log(core::LogLevel::Info, "session", std::format("started {} for {} as pid {}", session.name, user.name, pid));
  int status = 0;
